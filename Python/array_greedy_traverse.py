...
class array_traverse():
    ...
    def traverse_arr(self, inp_arr, i, j):
        print(inp_arr[i:,j:])
        if (self.down(inp_arr, i, j) > self.right(inp_arr, i, j) and i < self.row_max and j < self.col_max) or \
                (j == self.col_max and i < self.row_max):
            print("down we go", "new i:", i+1, "j: ", j, inp_arr[i+1][j])
            self.path.append(inp_arr[i+1][j])
            self.traverse_arr(inp_arr, i+1, j)
        elif (self.down(inp_arr, i, j) < self.right(inp_arr, i, j) and i < self.row_max and j < self.col_max) or \
                (i == self.row_max and j < self.col_max):
            print("right we go", "i:", i, "new j: ", j+1, inp_arr[i][j+1])
            self.path.append(inp_arr[i][j+1])
            self.traverse_arr(inp_arr, i, j+1)


i_a = np.array([[1,2,3,1,5],[4,9,6,4,3],[7,8,9,8,10],[3,4,5,1,9]])
tr = array_traverse()
